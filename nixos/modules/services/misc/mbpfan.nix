...
in
{
  ...
  options.services.mbpfan.silent = lib.mkEnableOption "less aggressive default fan speeds";
  ...
  options.services.mbpfan.settings = lib.mkOption {
    ...
    type = lib.types.submodule {
      ...
      options.general.low_temp = lib.mkOption {
        type = lib.types.int;
        default = (if cfg.silent then 63 else 55);
      ...
      options.general.high_temp = lib.mkOption {
        type = lib.types.int;
        default = (if cfg.silent then 66 else 58);
      ...
      options.general.max_temp = lib.mkOption {
        type = lib.types.int;
        default = (if cfg.silent then 86 else 78);
